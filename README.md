...
- generic type expression syntax such as `List[Int]`, `Option[Int]`, and `Map[String, Int]` is parsed; `List[T]`, list literals, basic list operations (`len`, `is_empty`, `push`, `get`, `set`), direct list indexing, `Option[T]`, `Option::Some`, `Option::None`, exhaustive Option `match`, and the first `Map[K, V]` slice (`Map.empty`, `len`, `is_empty`, `contains`, `get`, `insert`, `remove`) are implemented, while general enum declarations, `Result[T, E]`, map literals, and arbitrary map key types are not implemented yet
