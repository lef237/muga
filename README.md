...
- `record`, field access, `record.with` update, chained UFCS-style calls, local binding annotations, `List[T]` / `Option[T]` type annotations, list literals, safe list lookup, `Option::Some` / `Option::None`, exhaustive Option `match`, and arrow function type annotations are implemented
...
- generic type expression syntax such as `List[Int]` and `Option[Int]` is parsed; `List[T]`, list literals, basic list operations (`len`, `is_empty`, `push`, `get`), `Option[T]`, `Option::Some`, `Option::None`, and exhaustive Option `match` are implemented, while indexing, `set`, general enum declarations, and `Map[K, V]` are not implemented yet
