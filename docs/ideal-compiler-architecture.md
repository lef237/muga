...
- MIR is still expression-shaped, though it now has explicit execution bodies, body terminators, hoisted body-local function definitions, typed binding/package-item identity, and typed assignment update mode, and is not yet a control-flow-oriented backend IR
