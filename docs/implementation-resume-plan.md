...
- [x] interface summaries preserve public `TypeInfo`, package record identity, and collection types.
...
- [x] generic type expressions for compiler-known `List[T]`, `Option[T]`, and `Map[K, V]`.
...
- [ ] `Result[T, E]` and error propagation are not implemented.
...
- General IO, process, HTTP, time, and concurrency APIs need an error model before their APIs harden.
- Persisted package interfaces should not be frozen before enum/result types are represented in `TypeInfo` and public signatures.

## Requirement Decisions For The Next Slice

Decide these before coding the next feature:

- [ ] Whether general `enum` syntax lands before `Result[T, E]`, or whether `Result[T, E]` first lands as a compiler-known enum-like standard type.
...
- Implement `Result[T, E]` before `?` propagation sugar.
...
- [ ] New enum/Result behavior is covered in parser, typechecker, runtime, typed HIR, and package-interface tests as applicable.
- [ ] Exhaustiveness diagnostics include missing and duplicate variant coverage.
- [ ] Public signatures containing enum/result types are represented in package interface summaries.
