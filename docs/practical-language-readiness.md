...
1. `std::string`: the first prelude-helper slices implement `trim`, `contains`, `starts_with`, `ends_with`, `is_empty`, `replace`, `split`, `parse_int`, and `parse_bool`; next string work should decide substring/slice and leave `String.len()` plus richer parse error types until those semantics are explicit.
