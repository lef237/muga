...
- MIR is the backend boundary for default compile APIs, and bytecode/runtime name references now carry semantic `BindingId`, lowered `LocalId`, and display symbols for diagnostics; bytecode exposes local metadata for binding-backed and synthetic locals; runtime environments are slot-backed by `LocalId`
