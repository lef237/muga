...
- `Result[T, E]`, `Result::Ok`, `Result::Err`, and exhaustive Result `match` are implemented through the same compiler-known enum path
...
- local binding annotations, `List[T]` type annotations, list literals, list indexing, basic list operations including safe lookup and value-returning update, `Option[T]`, `Option::Some`, `Option::None`, exhaustive Option `match`, compiler-known `Result[T, E]`, `Result::Ok`, `Result::Err`, exhaustive Result `match`, and the first `Map[K, V]` slice are implemented; map literals, `Set[T]`, and broader collection APIs remain reserved
- AST/HIR/typed HIR now represent current `Option` / `Result` match arms as enum variant patterns, runtime Option/Result values use a generic enum value shape, and current Option/Result variant facts live in a compiler-known enum metadata table consumed by match checking, bytecode lowering, and runtime branching
