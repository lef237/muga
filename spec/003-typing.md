...
The current Rust implementation supports generic type expressions for compiler-known `List[T]`, `Option[T]`, and `Map[K, V]`. User-defined generic records and functions are still deferred. The generics MVP is specified in [009-generics.md](./009-generics.md).
