...
than into the entry source file, each manifest diagnostic also carries a
`source` context with role `manifest` naming the file the span belongs to. A
...
and must not use tool metadata directories such as `.git` or `.muga`.
...
For package content hashes and `.mgp` archive bytes, local source/resource
collection rejects symlinked source roots, resource roots, and entries instead
of following them outside the declared package boundary.
...
Local archive emitters must not follow symlinked source/resource roots or
entries, and must reject an archive output directory inside the declared source
or resource root so emitted archives cannot become part of their own package
input.
...
1. serialize and validate the complete new contents before touching the
   destination
2. write a uniquely named sibling temporary file without following symlinks
3. flush the file, and the parent directory where the supported host requires
   it for the documented durability guarantee
...
- emits deterministic `.mgp` source/resource archives through `emit-package-archive --archive-root <dir> <entry>`, naming the archive with the package name and content hash, rejecting archive output roots inside declared package source/resource roots, and printing a pasteable local archive dependency entry through `--dependency-snippet`
- reads and validates `.mgp` source/resource archive bytes through library APIs, computing the hash from the bytes, optionally checking an expected `sha256:<hex>`, and rejecting malformed or non-canonical manifest/source/resource entry layout plus unsafe archived manifest `source` or `resources` roots
